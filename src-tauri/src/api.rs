...
#[derive(Debug, Serialize, Deserialize)]
pub struct EchoPromptsResponse {
    prompts: Vec<EchoPrompt>,
    total: i32,
...
// Fetch Echo Prompts API
#[tauri::command]
pub async fn fetch_prompts() -> Result<EchoPromptsResponse, String> {
    let app_endpoint = get_app_endpoint()?;
    let api_access_key = get_api_access_key()?;
    ...
    let prompts_response: EchoPromptsResponse = response
        .json()
        .await
