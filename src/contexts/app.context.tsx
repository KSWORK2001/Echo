...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  // Legacy state kept for compatibility with existing context type.
  const [echoApiEnabled, setEchoApiEnabledState] = useState<boolean>(false);
  ...
  const loadData = () => {
    ...
    updateCursor(normalizedCustomizableState.cursor.type || "invisible");

    const stored = safeLocalStorage.getItem(STORAGE_KEYS.CUSTOMIZABLE);
    if (!stored) {
      // save the default state
      setCustomizableState(normalizedCustomizableState);
    } else {
      // check if we need to update the schema
      try {
        const parsed = JSON.parse(stored);
        if (!parsed.autostart || !parsed.alwaysOnTop?.isEnabled) {
          // save merged/normalized state with required defaults
          setCustomizableState(normalizedCustomizableState);
          updateCursor(normalizedCustomizableState.cursor.type || "invisible");
        }
      } catch (error) {
  ...
  useEffect(() => {
    const checkImageSupport = async () => {
      // For configured AI providers, check if curl contains {{IMAGE}}
  ...
  const setEchoApiEnabled = async (enabled: boolean) => {
    setEchoApiEnabledState(false);
    if (enabled) {
      console.warn("Legacy Echo API mode is disabled.");
    } else {
      const provider = allAiProviders.find(
        (p) => p.id === selectedAIProvider.provider
      );
