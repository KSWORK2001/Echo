...
const SELECTED_ECHO_MODEL_STORAGE_KEY = "selected_echo_model";
const SELECTED_ECHO_PROMPT_STORAGE_KEY = "selected_echo_prompt";

export const EchoPrompts = () => {
  const { setSystemPrompt, setSupportsImages, echoApiEnabled } = useApp();
  const [prompts, setPrompts] = useState<EchoPrompt[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [lastUpdated, setLastUpdated] = useState<string | null>(null);
  const [selectedEchoPrompt, setSelectedEchoPrompt] =
    useState<EchoPrompt | null>(() => {
      // Load selected prompt from local storage on initial render
      const stored = safeLocalStorage.getItem(
        SELECTED_ECHO_PROMPT_STORAGE_KEY
      );
      if (stored) {
        try {
  ...
  // Watch for changes in user's selected prompt and clear Echo selection if needed
  useEffect(() => {
    const checkUserPromptSelection = () => {
      const userSelectedPromptId = safeLocalStorage.getItem(
        STORAGE_KEYS.SELECTED_SYSTEM_PROMPT_ID
      );
      // If user has selected one of their own prompts, clear Echo prompt selection
      if (userSelectedPromptId) {
        setSelectedEchoPrompt(null);
      }
    };
  ...
  const fetchEchoPrompts = async () => {
    setIsLoading(true);
    setError(null);
    try {
      const response = await invoke<EchoPromptsResponse>("fetch_prompts");
      setPrompts(response.prompts);
      if (response.last_updated) {
        setLastUpdated(response.last_updated);
      }
    } catch (err) {
      console.error("Failed to fetch Echo prompts:", err);
      setError(typeof err === "string" ? err : "Failed to fetch Echo prompts");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSelectEchoPrompt = async (prompt: EchoPrompt) => {
    try {
      // Set the system prompt
      setSystemPrompt(prompt.prompt);
      setSelectedEchoPrompt(prompt);
      ...
      // Save the selected Echo prompt to local storage for persistence
      ...
      if (matchingModel) {
        ...
        await invoke("secure_storage_save", {
          items: [
            {
              key: SELECTED_ECHO_MODEL_STORAGE_KEY,
      ...
      console.error("Failed to select Echo prompt:", error);
    }
  };

  const handleCardClick = (prompt: EchoPrompt) => {
    handleSelectEchoPrompt(prompt);
  };

  const isPromptSelected = (prompt: EchoPrompt) => {
    return (
      selectedEchoPrompt?.title === prompt.title &&
      selectedEchoPrompt?.modelId === prompt.modelId
    );
  };
  ...
  if (isLoading) {
    return (
      <div className="space-y-4 mt-6">
        <Header
          title="Echo Default Personalities"
          description="Pre-configured personalities with optimal model selection"
        />
        <Empty
          isLoading={true}
          icon={Sparkles}
          title="Loading prompts..."
          description="Fetching Echo default personalities"
        />
      </div>
    );
  }
      ...
      <div className="flex items-start justify-between gap-3 border-t border-input/50 pt-6">
        <div className="flex items-start gap-3 w-full">
          <div className="flex flex-col gap-1 w-full">
            <Header
              title="Echo Default Personalities"
              description="Pre-configured personalities with optimal model pairings. Selecting a personality will automatically set the recommended AI model for best results."
            />
            {lastUpdated && (
